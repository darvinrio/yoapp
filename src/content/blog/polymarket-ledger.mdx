---
title: "Building a Polymarket Resolution dataset on Dune"
date: "2026-08-10"
description: "A technical documentation on rebuilding the Polymarket Ledger dataset on Dune"
tags: ["polymarket", "onchain data"]
---

import Image from "../../components/BlogImage.astro";
import ContractFlow from "../../assets/blog-images/polymarket-ledger/contract_control_flow.png";
import ConvertFlow from "../../assets/blog-images/polymarket-ledger/conv.png";
import LedgerHeader from "../../assets/blog-images/polymarket-ledger/ledger_header.png";

<Image src={LedgerHeader} alt="Ledger Header" />

> _AI DISCLOSURE: The idea and the prose was largely typed by a Human. However, AI was used to give a structure and a narrative to the blog post. You can find the different versions [here](https://github.com/darvinrio/polymarket-insider-heuristics/tree/main/reports/ledger)._
...
1. They are closed-source and black-boxed
...
> _A user can enter or exit positions through the CLOB and exit on Redemption._
...
- **Split**: A taker and maker jointly deposit USD, and receive the same amount of YES/NO tokens respectively.
- **Merge**: A taker and maker jointly deposit the same amount of YES/NO tokens, and receive USD.

<Image
  src={ContractFlow}
  alt="OrderMatching"
  caption="`OrderMatching` contract control flow"
/>
...
| 44      | **0x0c45c7c2b1ec281e2a8d0c204eb709f4bc9fba73** | 0x4bfb41d5b3570defd03c39a9a4d8de6bd8b8982e     | YES        | USD        | 10,000.000000     | 90.000000         | 10,000.000000 | $ 90.000000    | $ 0.09 | FullOrder |
...
This is problematic when we deal with a **Split** or **Merge** trade, as the maker's asset tokenID is opposite to the taker's. While the shares remain constant, since equal amounts are required for Collateral movement, the USD amount logged is different. This is why the sum of USD logged in the individual fills does not match the USD logged in the final **FullOrder** `OrderFilled` event. When trying to analyze the spread of execution, we will only have the average price of the trade, similar to that of the Polymarket API.
...
Looking back at the `0xce296aaf92ecc022cc6608a54c622bb1c445b71b` `Will Gemini 3.0 be released on November 17 2025?` market example:
Here is how the actual swap looks.
...
In the Reconstructing CLOB trades section, we computed the taker side of CLOB trades. However, for Ledger events, we only need the maker side. This is because the **FullOrder** trade captures the taker-side delta as the maker.
...
Our naive approach manages to match the expected pricing, but offsets the effective cost of YES tokens to the NO tokens. Instead of a single 9 USD for 10 YES tokens, we get 5 USD for 10 YES tokens and 5 USD for NO tokens sold on the market for 1 USD. Instead of a 9 USD YES position at 0 USD profit, we get two positions - a YES position at 4 USD profit and a NO position at 4 USD loss.
...
<Image
  src={ConvertFlow}
  alt="Convert Event Emissions"
  caption="Fig. 2 — `convertPositions` flow. The right-hand table shows event offsets relative to the terminal `PositionsConverted` event (`i`) across four fee/collateral configurations. For the no-fee, v1 case used here: the trader's NO burn (`BatchTransfer`) falls between `i-3` and `i-6`; the YES mint to the trader is always at `i-1`."
/>
...
For the `NO` burning leg, based on the figure above, the `BatchTransfer` is always between `i-3` and `i-6`, if `i` is the index of the `PositionConverted` event. This event offset is fee-schedule-dependent. Fig above shows the offset shifts under the "Both Fee & Collateral" and "No Collateral" configurations. An interesting takeaway is that we can determine the type of convert, based on the difference between the indexes of the `PositionConverted` and `BatchTransfer` events.
...
| Split                  | `$ -usd`                          | `shares`                                   | `$0.5`                 | $1 of collateral mints 1 YES + 1 NO — priced at parity                                           |
| Merge                  | `$ +usd`                          | `-shares`                                  | `$0.5`                 | 1 YES + 1 NO burns for $1 of collateral — same parity                                            |
...
The logic is that, since all positions must be ERC 1155 positions, the balance ledger should match the number of positions in our ledger.
...
Of the 1166 positions that did not match the API
...
2. `0x01542a212c9696da5b409cae879143b8966115a8` wallet `YES` on `Will Bitcoin reach $105,000 in January?` market has 405 transfer events, but the API has only logged the `NO` position, which itself has a `$-307,989.28` PnL. In this case, the positive PnL of the YES position would register a net positive PnL.
...
- How much time before an execution price is now considered stale?
...
- ERC 1155 Ledger Summary - [Dune Query](https://dune.com/queries/8230946)
